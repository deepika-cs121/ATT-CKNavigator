// This file is now ONLY for the frontend TypeScript types
// and providing the node data for the visualizer.
...
export interface AttackStep {
  ...
  visual: {
    from: string; // Node ID
    to: string;   // Node ID
    action: string; // Text to display
  };
  redLog: AttackLogEntry;
  blueLog: AttackLogEntry | null; // Some steps might not have a blue log
}

export interface ScenarioNode {
  id: string;   // Unique ID (e.g., 'attacker', 'firewall')
...
// --- SCENARIO DATABASE (for frontend use) ---
// We keep this here so the frontend can immediately show the nodes
// when an attack is selected, before the backend even starts.
// The backend has its own copy of this logic.
...
// A default scenario for any missing tactics
export const getDefaultScenario = (tactic: string): AttackScenario => ({
  techniqueId: 'T0000',
