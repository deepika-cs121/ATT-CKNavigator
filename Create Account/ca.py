#!/usr/bin/env python3
"""
Cloud Account Creation Simulator & Detection Log
...
class CloudAccountSimulator:
    ...
    def get_user_input(self) -> RunConfig:
        print("=== CLOUD ACCOUNT CREATION SIMULATOR ===")
        print("Authorized Use Only – Requires Written Permission")
        print("=" * 45)

        cloud_provider = input("Enter cloud provider (e.g., aws, azure): ").strip().lower()
        ...
        return RunConfig(
            cloud_provider=cloud_provider, base_username=base_username,
    ...
    def run_attack(self):
        print(f"\nStarting Cloud Account Creation Simulation on {self.config.cloud_provider.upper()}")
    ...
    def cleanup(self):
        ...
        if self.created_accounts:
            print("\n=== CREATED CLOUD ACCOUNTS ===")
            for acc in self.created_accounts:
                print(f"[+] Username: {acc['username']}@{self.config.cloud_provider}")
            print("=======================================")
        else:
            print("No cloud accounts created during this run.")
...
if __name__ == "__main__":
    simulator = CloudAccountSimulator()
    simulator.run()