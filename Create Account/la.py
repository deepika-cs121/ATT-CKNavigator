#!/usr/bin/env python3
"""
Local Account Creation Simulator & Detection Log
...
class LocalAccountSimulator:
    ...
    def get_user_input(self) -> RunConfig:
        print("=== LOCAL ACCOUNT CREATION SIMULATOR ===")
        print("Authorized Use Only – Requires Written Permission")
        print("=" * 45)

        base_username = input("Enter base username (e.g., backdoor): ").strip()
        group = input("Enter group to add user to (e.g., sudo): ").strip()
        try:
        ...
        return RunConfig(
            base_username=base_username, group=group, max_attempts=max_attempts,
    ...
    def run_attack(self):
        print(f"\nStarting Local Account Creation Simulation for base: {self.config.base_username}")
    ...
    def cleanup(self):
        ...
        summary = {
            "run_id": self.run_id,
            "base_username": self.config.base_username,
            "group": self.config.group,
        ...
        if self.created_accounts:
            ...
            print("No accounts created during this run.")
...
if __name__ == "__main__":
    simulator = LocalAccountSimulator()
    simulator.run()